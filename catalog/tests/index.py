...
class CatalogIndexTests(TestCase):
    ...
    def test_cat_to_class(self):
        ...
        self.assertIn("Edition", book_classes)
        self.assertIn("Work", book_classes)

        movie_classes = _cat_to_class(ItemCategory.Movie.value)
        self.assertIn("Movie", movie_classes)
...
class CatalogQueryParserTests(TestCase):
    databases = "__all__"
    ...
    def test_init_basic(self):
        ...
        self.assertEqual(parser.q, "test query")
        self.assertEqual(parser.page, 1)
        self.assertEqual(parser.page_size, 20)
        self.assertEqual(parser.filter_by, {})
    ...
    def test_tag_filtering(self):
        ...
        self.assertEqual(
            sorted(parser.filter_by.get("tag", [])), sorted(["scifi", "fantasy"])
        )
    ...
    def test_category_filtering(self):
        ...
        self.assertEqual(parser.q, "")
        self.assertIn("item_class", parser.filter_by)

        # Should include Edition, Work and Movie classes
        classes = parser.filter_by["item_class"]
        self.assertIn("Edition", classes)
        self.assertIn("Work", classes)
        self.assertIn("Movie", classes)
    ...
    def test_exclude_categories(self):
        ...
        self.assertEqual(parser.q, "test")
        self.assertIn("item_class", parser.exclude_by)

        # Should exclude Edition, Work, and Movie classes
        excluded = parser.exclude_by["item_class"]
        self.assertIn("Edition", excluded)
        self.assertIn("Work", excluded)
        self.assertIn("Movie", excluded)
    ...
    def test_filter_categories_precedence(self):
        ...
        self.assertEqual(parser.q, "test")
        self.assertIn("item_class", parser.filter_by)
        self.assertNotIn("item_class", parser.exclude_by)
