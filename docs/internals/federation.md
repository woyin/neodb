...
NeoDB is not a PDS itself currently, but can interact with a PDS to import the user's social graph and send status updates. So technically NeoDB does not do full federation in ATProto, but NeoDB will handle some side effects from federation. For example, when a user logs in via an ATProto handle, NeoDB will resolve the user's DID and store it, attempt further operations with the DID, update the user's handle if it has changed, and use the corresponding PDS for that handle. The user may still have to log in to NeoDB again with their Bluesky app password, since a change of PDS may invalidate the previous app password.
