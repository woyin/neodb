...
// Shikwasa keeps .shk-controls_extra collapsed behind the "more" toggle, so a
// button parked there costs two taps. Give that slot to speed and put the mark
// button on the always-visible row. Speed is not registered with
// hideExtraControl(), which would close the panel 800ms after every tap and
// make cycling 1.0x/1.5x/2.0x impossible.
function swap_podcast_mark_and_speed(comment_btn) {
  var ui = window.player.ui;
  var basic_controls = ui.el.querySelector('.shk-controls_basic');
  if (comment_btn) {
    basic_controls.insertBefore(comment_btn, ui.bwdBtn);
    ui.extraControls.append(ui.speedBtn);
  } else {
    basic_controls.prepend(ui.speedBtn);
  }
}
function set_podcast_comment_button(comment_href) {
  ...
  if (comment_href) {
    ...
    swap_podcast_mark_and_speed(window.comment_btn);
    ...
    swap_podcast_mark_and_speed(null);
  }
}
